...
# In-memory storage with reservations and cancellation tracking
tables = [
...
<body>
    <div class="container">
        <h2>🍽️ DarkFactory - Restaurant Reservations</h2>
...
# Route 1: Graphical Web UI at root URL
@app.route('/', methods=['GET'])
def index():
    return render_template_string(TEMPLATE, tables=tables, message=None)

# Route 2: Handle Web UI Form Submission
...
# Route 3: JSON API for Tables (Stage 4 / API clients)
@app.route('/api/tables', methods=['GET'])
def get_tables():
    return jsonify({"success": True, "tables": tables}), 200

# Route 4: JSON API for Reservations with Idempotency & Concurrency
...
# Route 5: Cancel Reservation (DELETE API)
